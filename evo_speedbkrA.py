...
try:
 with open('save/save5.txt', 'rb') as f:
...
#이 위로 수정 ㄴ
r = session.get("https://babylonvg.evo-games.com/frontend/evo/r2/#category=baccarat_sicbo&game=baccarat&table_id=leqhceumaq6qfoug", headers=headers)
...
def on_message1(ws1, message):
        ...
        if "dealer.changed" in message:
            ...
            try:
              webhook = DiscordWebhook(url=에볼루션스피드바카라A, username="스피드 바카라 A")
              embed = DiscordEmbed(title="**스피드 바카라 A**", description="딜러 변경 : "+str(wi1))
           ...
           try:
              ...
              webhook = DiscordWebhook(url=에볼루션스피드바카라A, username=id)
              embed = DiscordEmbed(title="**스피드 바카라 A**", description="**```d\nPlayer : "+str(pscore)+f"\n{pformattedcard}\nㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ\nBanker : "+str(bscore)+f"\n{bformattedcard}```**")
              webhook.add_embed(embed)
              response = webhook.execute()
            ...
            try:
              ...
              for i in list_a:
                ...
                if i[74] == ukor:
                  ...
                  embed = { 'title': f'적중', 'description': f'```배팅 게임 : 에볼루션 스피드바카라 A\n배팅 내역 : {i[74]}\n배팅 금액 : {i[75]}원\n─────────────\n적중 금액 : {round(i[75] * (배당-1))}\n남은 금액 : {i[1] + round(i[75] * 배당)}```', 'color' : 0x00FF00}
                  req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                  headers=headers, json={
                  'content' : f"<@{i[0]}> ", 'embed' : embed})
                  c.execute("UPDATE users SET money = ? WHERE id == ?;", (i[1] + round(i[75] * 배당), i[0]))
                
                elif i[74] == pp or i[74] == bp:
                    headers = {
                        "Authorization": f"Bot {봇토큰}"
                    }
                    js = {
                    ...
                    embed = { 'title': f'적중', 'description': f'```배팅 게임 : 에볼루션 코리안스피드바카라 A\n배팅 내역 : {i[86]}\n배팅 금액 : {i[87]}원\n─────────────\n적중 금액 : {round(i[87] * 11)}\n남은 금액 : {i[1] + round(i[87] * 12)}```', 'color' : 0x00FF00}
                    req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                    headers=headers, json={
                    'content' : f"<@{i[0]}> ", 'embed' : embed})
                  ...
                  if ukor == "타이":
                    ...
                    data = res.json()
                    dm_channel_id = data['id']
                    embed = { 'title': f'무승부', 'description': f'```배팅 게임 : 에볼루션 스피드바카라 A\n배팅 내역 : {i[74]}\n배팅 금액 : {i[75]}원\n─────────────\n적중 금액 : 0\n남은 금액 : {i[1] + i[75]}```', 'color' : 0xFF0000}
                    req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                    headers=headers, json={
                    'content' : f"<@{i[0]}> ", 'embed' : embed})
                    c.execute("UPDATE users SET money = ? WHERE id == ?;", (i[1] + round(i[75]), i[0]))
                  else:
                    headers = {
                        "Authorization": f"Bot {봇토큰}"
                    }
                    ...
                    data = res.json()
                    dm_channel_id = data['id']
                    embed = { 'title': f'미적중', 'description': f'```배팅 게임 : 에볼루션 스피드바카라 A\n배팅 내역 : {i[74]}\n배팅 금액 : {i[75]}원\n─────────────\n적중 금액 : 0\n남은 금액 : {i[1]}```', 'color' : 0xFF0000}
                    req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                    headers=headers, json={
                    'content' : f"<@{i[0]}> ", 'embed' : embed})
                    
                c.execute("UPDATE users SET evobkrA_pick = ? where id=?", (None, i[0],))
                c.execute("UPDATE users SET evobkrA_money = ? where id=?", (None, i[0],))
                conn.commit()
                conn.close()      
            except Exception as e:
              print(e)
            ...
            if(tasty == 'Dealing'):
                ...
                try:
                  webhook = DiscordWebhook(url=에볼루션스피드바카라A, username=id)
                  embed = DiscordEmbed(title="**스피드 바카라 A**", description="**```diff\n- 배팅마감, 딜러가 딜링을 시작합니다.```**")
                  webhook.add_embed(embed)
                  response = webhook.execute()
                except:
                   pass   
             ...
             #딜링이 끝났고 베팅이 가능한 상태입니다.
              ...
              try:
                webhook = DiscordWebhook(url=에볼루션스피드바카라A, username=id)
                embed = DiscordEmbed(title="**스피드 바카라 A**", description=f"**```diff\n+ 딜링이 끝났습니다, 배팅이 가능하십니다.```**")
                embed.set_image(url="attachment://grid.png")
                webhook.add_embed(embed)
                response = webhook.execute()
                webhook_url = 에볼루션스피드바카라A

                with open(image_path, "rb") as image_file:
                    image_data = image_file.read()

                payload = {
                    "file": ("grid.png", image_data)
                }
...
def wsjoa1():
 ...
 ws1 = websocket.WebSocketApp('wss://babylonvg.evo-games.com/public/baccarat/player/game/leqhceumaq6qfoug/socket?messageFormat=json&instance=bt1fz-'+str(oki)+'-leqhceumaq6qfoug&tableConfig=&EVOSESSIONID='+str(cookies['EVOSESSIONID'])+'&client_version=6.20230327.72627.23195-882f9014ff',
                       header=headers,
