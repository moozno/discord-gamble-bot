...
while True:
    try:
        # res = requests.get("https://bepick.net/live/result/jw_angelsdemons?_="+str(time.time()).split(".")[0], headers={
        ...
        old_data = {}
        resp=res
        with open("./angelsdemons.json", "r", encoding="utf8") as fp:
            old_data = json.loads(fp.read().strip())
        if( old_data == res ):
            pass
        else:
            with open("./angelsdemons.json", "w", encoding="utf8") as fp:
               # ow=int(res["fd1"])
              ...
              #  if user_info[12]!=None:
                ...
                fields = []
                fields.append({'name': '천사/악마', 'value': f'{ "악마" if int(res["fd1"]) % 2 == 0 else "천사" }'})
                embed = { 'title': '천사와악마 결과', 'description': f'{res["round"]}회차', 'fields': fields }
                requests.post(천사와악마, json={'username': f'{res["round"]}회차 천사와악마 결과', "embeds": [embed]})
        ...
        # print(res)
                ...
                conn.close()
                ow = "악마" if int(res["fd1"]) % 2 == 0 else "천사"
                for i in list_a:
                    if(i[32] == None):
                        # print("none")
                        continue
                    ...
                    if(i[32] == ow):
                        ...
                        embed = { 'title': f'적중', 'description': f'```배팅 게임 : 천사와악마\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[32]}\n배팅 금액 : {i[33]}원\n─────────────\n적중 금액 : {round(i[33] * 0.98)}\n남은 금액 : {i[1] + round(i[33] * 1.98)}```', 'color' : 0x00FF00}
                        req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                        headers=headers, json={
                        'content' : f"<@{i[0]}> ", 'embed' : embed})
                        c.execute("UPDATE users SET money = money + ? where id=?", (round(i[33] * 1.98), i[0],))
    ...
    ''')
                        ...
                        data = res.json()
                        dm_channel_id = data['id']
                        embed = { 'title': f'미적중', 'description': f'```배팅 게임 : 천사와악마\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[32]}\n배팅 금액 : {i[33]}원\n─────────────\n적중 금액 : 0\n남은 금액 : {i[1]}```', 'color' : 0xFF0000}
                        req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                        headers=headers, json={
                        'content' : f"<@{i[0]}> ", 'embed' : embed})
                        f = open(f"./bet_log/{i[0]}.txt", "a", encoding="utf-8-sig")
    ...
    ''')
                        f.close()
                    c.execute("UPDATE users SET mario_bet_pick = ? where id=?", (None, i[0],))
                    c.execute("UPDATE users SET mario_bet_money = ? where id=?", (None, i[0],))
                    conn.commit()
                    conn.close()
