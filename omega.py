...
while True:
    try:
        # res = requests.get("https://bepick.net/live/result/jp_alphaomega?_="+str(time.time()).split(".")[0], headers={
        ...
        url = 'https://bepick.net/live/result/jp_alphaomega'
        ...
        old_data = {}
        resp=res
        with open("./omega.json", "r", encoding="utf8") as fp:
            old_data = json.loads(fp.read().strip())
        if( old_data == res ):
            pass
        else:
            with open("./omega.json", "w", encoding="utf8") as fp:
               # ow=int(res["fd1"])
              ...
              #  if user_info[12]!=None:
                ...
                fields = []
                fields.append({'name': '알파 1,2', 'value': f'{res["op1"]} | {res["op2"]}'})
                fields.append({'name': '오메가 1,2', 'value': f'{res["op3"]} | {res["op4"]}'})
                fields.append({'name': '알파/오메가', 'value': f'{ "오메가" if int(res["fd1"]) % 2 == 0 else "알파" }'})
                embed = { 'title': '알파오메가 결과', 'description': f'{res["round"]}회차', 'fields': fields }
                requests.post(알파오메가, json={'username': f'{res["round"]}회차 알파오메가 결과', "embeds": [embed]})
        ...
        # print(res)
                ...
                conn.close()
                ow = "오메가" if int(res["fd1"]) % 2 == 0 else "알파"
                for i in list_a:
                    if(i[62] == None):
                        # print("none")
                        continue
                    ...
                    if(i[62] == ow):
                        ...
                        embed = { 'title': f'적중', 'description': f'```배팅 게임 : 알파오메가\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[62]}\n배팅 금액 : {i[63]}원\n─────────────\n적중 금액 : {round(i[63] * 0.98)}\n남은 금액 : {i[1] + round(i[63] * 1.98)}```', 'color' : 0x00FF00}
                        req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                        headers=headers, json={
                        'content' : f"<@{i[0]}> ", 'embed' : embed})
                        c.execute("UPDATE users SET money = money + ? where id=?", (round(i[63] * 1.98), i[0],))
    ...
    ''')
                        ...
                        data = res.json()
                        dm_channel_id = data['id']
                        embed = { 'title': f'미적중', 'description': f'```배팅 게임 : 알파오메가\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[62]}\n배팅 금액 : {i[63]}원\n─────────────\n적중 금액 : 0\n남은 금액 : {i[1]}```', 'color' : 0xFF0000}
                        req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                        headers=headers, json={
                        'content' : f"<@{i[0]}> ", 'embed' : embed})
                        f = open(f"./bet_log/{i[0]}.txt", "a", encoding="utf-8-sig")
    ...
    ''')
                        f.close()
                    c.execute("UPDATE users SET mega_pick = ? where id=?", (None, i[0],))
                    c.execute("UPDATE users SET mega_money = ? where id=?", (None, i[0],))
                    conn.commit()
                    conn.close()
