...
while True:
    try:
        # res = requests.get("https://bepick.net/live/result/duyenha?_="+str(time.time()).split(".")[0], headers={
        ...
        old_data = {}
        resp=res
        with open("./duyenha.json", "r", encoding="utf8") as fp:
            old_data = json.loads(fp.read().strip())
        if( old_data == res ):
            pass
        else:
            with open("./duyenha.json", "w", encoding="utf8") as fp:
               # ow=int(res["fd1"])
              ...
              #  if user_info[12]!=None:
                ...
                if int(res["fd1"]) == 1:
                    ...
                    ow = "타이"
                fields = []
                fields.append({'name': '플레이어 카드', 'value': f'{res["p1"]} / {res["p2"]} / {res["p3"]}'})
                fields.append({'name': '뱅커 카드', 'value': f'{res["b1"]} / {res["b2"]} / {res["b3"]}'})
                fields.append({'name': '플/뱅/무', 'value': f'{ow}'})
                embed = { 'title': '두옌하바카라 결과', 'description': f'{res["round"]}회차', 'fields': fields }
                requests.post(두옌하바카라, json={'username': f'{res["round"]}회차 두옌하바카라 결과', "embeds": [embed]})
                ...
                for i in list_a:
                    ...
                    if(i[46] == ow):
                        if ow == "타이":
                            ...
                            embed = { 'title': f'적중', 'description': f'```배팅 게임 : 두옌하바카라\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[46]}\n배팅 금액 : {i[47]}원\n─────────────\n적중 금액 : {round(i[47] * 7)}\n남은 금액 : {i[1] + round(i[47] * 8)}```', 'color' : 0x00FF00}
                            req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                            headers=headers, json={
                            'content' : f"<@{i[0]}> ", 'embed' : embed})
                            c.execute("UPDATE users SET money = money + ? where id=?", (round(i[47] * 8), i[0],))
        ...
        적중 / 미적중 : 적중
        적중 금액 : {round(i[47] * 7)}
        남은 금액 : {i[1] + round(i[47] * 8)}
        ======================
        ''')
                            f.close()
                            c.execute("UPDATE users SET bakara_pick = ? where id=?", (None, i[0],))
                            c.execute("UPDATE users SET bakara_money = ? where id=?", (None, i[0],))
                            conn.commit()
                            conn.close()
                        else:
                            headers = {
                            ...
                            data = res.json()
                            dm_channel_id = data['id']
                            embed = { 'title': f'적중', 'description': f'```배팅 게임 : 두옌하바카라\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[46]}\n배팅 금액 : {i[47]}원\n─────────────\n적중 금액 : {round(i[47] * 0.98)}\n남은 금액 : {i[1] + round(i[47] * 1.98)}```', 'color' : 0x00FF00}
                            req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                            headers=headers, json={
                            'content' : f"<@{i[0]}> ", 'embed' : embed})
                            c.execute("UPDATE users SET money = money + ? where id=?", (round(i[47] * 1.98), i[0],))
                            f = open(f"./bet_log/{i[0]}.txt", "a", encoding="utf-8-sig")
                            f.write(
                                f'''                
        배팅게임 : 두옌하바카라
        ...
        ''')
                        ...
                        if ow == "타이":
                            ...
                            data = res.json()
                            dm_channel_id = data['id']
                            embed = { 'title': f'타이', 'description': f'```배팅 게임 : 두옌하바카라\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[46]}\n배팅 금액 : {i[47]}원\n─────────────\n적중 금액 : 0\n남은 금액 : {i[1] + round(i[47] * 1)}```', 'color' : 0x00FF00}
                            req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                            headers=headers, json={
                            'content' : f"<@{i[0]}> ", 'embed' : embed})
                            c.execute("UPDATE users SET money = money + ? where id=?", (round(i[47] * 1), i[0],))
                            f = open(f"./bet_log/{i[0]}.txt", "a", encoding="utf-8-sig")
                            f.write(
                                f'''                
        배팅게임 : 두옌하바카라
        ...
        ''')
                            ...
                            data = res.json()
                            dm_channel_id = data['id']
                            embed = { 'title': f'미적중', 'description': f'```배팅 게임 : 두옌하바카라\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[46]}\n배팅 금액 : {i[47]}원\n─────────────\n적중 금액 : 0\n남은 금액 : {i[1]}```', 'color' : 0xFF0000}
                            req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                            headers=headers, json={
                            'content' : f"<@{i[0]}> ", 'embed' : embed})
                            f = open(f"./bet_log/{i[0]}.txt", "a", encoding="utf-8-sig")
