...
while True:
    try:
        # res = requests.get("https://bepick.net/live/result/bubble_power?_="+str(time.time()).split(".")[0], headers={
        ...
        resp =res
        old_data = {}
        with open("./bubble_power_temp.json", "r", encoding="utf8") as fp:
            old_data = json.loads(fp.read().strip())
        if( old_data == res ):
            pass
        else:
            with open("./bubble_power_temp.json", "w", encoding="utf8") as fp:
                bg_o=int(res["bsum"])
                bg_p=int(res["pb"])
                ...
                fields.append({'name': '파워볼', 'value': f'{ "파짝" if bg_p % 2 == 0 else "파홀" } / { "파언" if bg_p < 5 else "파옵" }'})
                ...
                embed = { 'title': '보글 파워볼 결과', 'description': f'{res["round"]}회차', 'fields': fields }
                requests.post(보글볼, json={'username': f'{res["round"]}회차 보글볼 결과', "embeds": [embed]})
        # print(res)
                ...
                conn.close()
                #pa_z = "파짝" if int(res["pb"]) % 2 == 0 else "파홀"

                #pa_o = "파언" if int(res["pb"]) < 5 else "파옵"

                #ill_z = "일짝" if int(res["bsum"]) % 2 == 0 else "일홀"

                #ill_o = "일언" if int(res["bsum"]) < 73 else "일옵"
                pa_z = "파짝" if bg_p % 2 == 0 else "파홀"

                pa_o = "파언" if bg_p < 5 else "파옵"

                ill_z = "일짝" if bg_o % 2 == 0 else "일홀"

                ill_o = "일언" if bg_o < 73 else "일옵"
                ...
                for i in list_a:
                    ...
                    if i[0]!=687467109243945121:
                        #print(f"{pa_z} {pa_o} {ill_z} {ill_o}")
                        #print(i[2] == pa_z, i[2] == pa_o, i[2] == ill_z, i[2] == ill_o)
                        ...
                        if(i[2] == pa_z or i[2] == pa_o or i[2] == ill_z or i[2] == ill_o or i[2] == sjd):
                            headers = {
                            ...
                            embed = { 'title': f'적중', 'description': f'```배팅 게임 : 보글파워볼\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[2]}\n배팅 금액 : {i[3]}원\n─────────────\n적중 금액 : {round(i[3] * 0.98)}\n남은 금액 : {i[1] + round(i[3] * 1.98)}```', 'color' : 0x00FF00}
                            req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                            headers=headers, json={
                            'content' : f"<@{i[0]}> ", 'embed' : embed})
                            c.execute("UPDATE users SET money = money + ? where id=?", (round(i[3] * 1.98), i[0],))
        ...
        ''')
                            ...
                            data = res.json()
                            dm_channel_id = data['id']
                            embed = { 'title': f'미적중', 'description': f'```배팅 게임 : 보글파워볼\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[2]}\n배팅 금액 : {i[3]}원\n─────────────\n적중 금액 : 0\n남은 금액 : {i[1]}```', 'color' : 0xFF0000}
                            req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                            headers=headers, json={
                            'content' : f"<@{i[0]}> ", 'embed' : embed})
                            f = open(f"./bet_log/{i[0]}.txt", "a", encoding="utf-8-sig")
        ...
        ''')
                    ...
                    c.execute("UPDATE users SET boggle_bet_pick = ? where id=?", (None, i[0],))
                    c.execute("UPDATE users SET boggle_bet_money = ? where id=?", (None, i[0],))
                    conn.commit()
                    conn.close()
