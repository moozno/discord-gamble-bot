...
try:
 with open('save/save2.txt', 'rb') as f:
...
if('200' not in r):
 ...
 with open('save/save2.txt', 'wb') as f:
...
#이 위로 수정 ㄴ
r = session.get("https://babylonvg.evo-games.com/frontend/evo/r3/#category=baccarat_sicbo&game=baccarat&table_id=onokyd4wn7uekbjx", headers=headers)
...
def on_message1(ws1, message):
        ...
        if "dealer.changed" in message:
            ...
            try:
              webhook = DiscordWebhook(url=코리아바카라B, username="코리안 바카라 B")
              embed = DiscordEmbed(title="**코리안 바카라 B**", description="딜러 변경 : "+str(wi1))
           ...
           try:
              ...
              webhook = DiscordWebhook(url=코리아바카라B, username=id)
              embed = DiscordEmbed(title="**코리안 바카라 B**", description="**```d\nPlayer : "+str(pscore)+f"\n{pformattedcard}\nㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ\nBanker : "+str(bscore)+f"\n{bformattedcard}```**")
              webhook.add_embed(embed)
              response = webhook.execute()
              ...
              for i in list_a:
                ...
                if i[86] == ukor:
                  ...
                  if ukor == "타이":
                    ...
                    data = res.json()
                    dm_channel_id = data['id']
                    embed = { 'title': f'무승부', 'description': f'```배팅 게임 : 에볼루션 코리안스피드바카라 A\n배팅 내역 : {i[86]}\n배팅 금액 : {i[87]}원\n─────────────\n적중 금액 : 0\n남은 금액 : {i[1] + i[87]}```', 'color' : 0xFF0000}
                    req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                    headers=headers, json={
                    'content' : f"<@{i[0]}> ", 'embed' : embed})
                    c.execute("UPDATE users SET money = ? WHERE id == ?;", (i[1] + round(i[87]), i[0]))
                  else:
                    headers = {
                        "Authorization": f"Bot {봇토큰}"
                    }
                ...
                c.execute("UPDATE users SET evokoreabkrB_pick = ? where id=?", (None, i[0],))
                c.execute("UPDATE users SET evokoreabkrB_money = ? where id=?", (None, i[0],))
                conn.commit()
                conn.close()      
            except Exception as e:
                print(e)  
            ...
            if(tasty == 'Dealing'):
                ...
                try:
                  webhook = DiscordWebhook(url=코리아바카라B, username=id)
                  embed = DiscordEmbed(title="**코리안 바카라 B**", description="**```diff\n- 배팅마감, 딜러가 딜링을 시작합니다.```**")
                  webhook.add_embed(embed)
                  response = webhook.execute()
...
def wsjoa1():
 ...
 ws1 = websocket.WebSocketApp('wss://babylonvg.evo-games.com/public/baccarat/player/game/onokyd4wn7uekbjx/socket?messageFormat=json&instance=bt1fz-'+str(oki)+'-leqhceumaq6qfoug&tableConfig=&EVOSESSIONID='+str(cookies['EVOSESSIONID'])+'&client_version=6.20230327.72627.23195-882f9014ff',
                       header=headers,
