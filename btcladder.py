...
while True:
    try:
        # res = requests.get("https://bepick.net/live/result/btc_ladder?_="+str(time.time()).split(".")[0], headers={
        ...
        resp=res
        old_data = {}
        with open("./btc_ladder.json", "r", encoding="utf8") as fp:
            old_data = json.loads(fp.read().strip())
        if( old_data == res ):
            pass
        else:
            with open("./btc_ladder.json", "w", encoding="utf8") as fp:
    #             fp.write(json.dumps(res))
    #             data = {
    #                 "username": f'{res["round"]}'"회차 비트사다리 결과",
    #                 "content": f'''
    # ```py
    # [ 비트사다리 파워볼 결과 ]
    ...
    #     # print(res)
                fp.write(json.dumps(res))
                fields = []
                fields.append({'name': '비트사다리', 'value': f'{ "우" if int(res["fd1"]) % 2 == 0 else "좌" } | {"사" if int(res["fd2"]) % 2 == 0  else "삼" } | {"짝" if int(res["fd3"]) % 2 == 0  else "홀" }'})
                embed = { 'title': '비트사다리 결과', 'description': f'{res["round"]}회차', 'fields': fields }
                requests.post(비트코인사다리, json={'username': f'{res["round"]}회차 비트사다리 결과', "embeds": [embed]})
        # print(res)
                ...
                for i in list_a:
                    ...
                    if(i[6] == rightle or i[6] == fourthree or i[6] == oddeven):
                        headers = {
                        ...
                        embed = { 'title': f'적중', 'description': f'```배팅 게임 : 빗썸사다리\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[6]}\n배팅 금액 : {i[7]}원\n─────────────\n적중 금액 : {round(i[7] * 0.98)}\n남은 금액 : {i[1] + round(i[7] * 1.98)}```', 'color' : 0x00FF00}
                        req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                        headers=headers, json={
                        'content' : f"<@{i[0]}> ", 'embed' : embed})
                        c.execute("UPDATE users SET money = money + ? where id=?", (round(i[7] * 1.98), i[0],))
    ...
    ''')
                        ...
                        data = res.json()
                        dm_channel_id = data['id']
                        embed = { 'title': f'미적중', 'description': f'```배팅 게임 : 빗썸사다리\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[6]}\n배팅 금액 : {i[7]}원\n─────────────\n적중 금액 : 0\n남은 금액 : {i[1]}```', 'color' : 0xFF0000}
                        req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                        headers=headers, json={
                        'content' : f"<@{i[0]}> ", 'embed' : embed})
                        f = open(f"./bet_log/{i[0]}.txt", "a", encoding="utf-8-sig")
    ...
    ''')
                    ...
                    c.execute("UPDATE users SET ladder_bet_pick = ? where id=?", (None, i[0],))
                    c.execute("UPDATE users SET ladder_bet_money = ? where id=?", (None, i[0],))
                    conn.commit()
                    conn.close()
