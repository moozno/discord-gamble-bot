...
while True:
    try:
        # res = requests.get("https://bepick.net/live/result/ntry_power?_="+str(time.time()).split(".")[0], headers={
        ...
        old_data = {}
        resp=res
        with open("./powerball.json", "r", encoding="utf8") as fp:
            old_data = json.loads(fp.read().strip())
        if( old_data == res ):
            pass
        else:
            with open("./powerball.json", "w", encoding="utf8") as fp:
                pw_o=int(res["bsum"])
                pw_p=int(res["pb"])
                ...
                fields.append({'name': '파워볼', 'value': f'{ "파짝" if pw_p % 2 == 0 else "파홀" } / { "파언" if pw_p < 5 else "파옵" }'})
                fields.append({'name': '일반볼', 'value': f'{ pw_o } / { "일짝" if pw_o % 2 == 0 else "일홀" } / { "일언" if pw_o < 73 else "일옵" } / { "소" if res["b5"] == "1" else  "중" if res["b5"] == "2" else "대"  }'})
                embed = { 'title': '파워볼 결과', 'description': f'{res["round"]}회차', 'fields': fields }
                requests.post(파워볼, json={'username': f'{res["round"]}회차 파워볼 결과', "embeds": [embed]})

                conn = sqlite3.connect('./database/database.db')
                c = conn.cursor()
                list_a = list(c.execute("SELECT * FROM users"))
                conn.close()
                pw_p = "파짝" if pw_p % 2 == 0 else "파홀"

                pw_o = "파언" if pw_p < 5 else "파옵"

                pw_z1 = "일짝" if pw_o % 2 == 0 else "일홀"

                pw_o1 = "일언" if pw_o < 73 else "일옵"
                for i in list_a:
                    if(i[14] == None):
                        # print("none")
                        continue
                    ...
                    #c = conn.cursor()
                        ...
                        if(i[14] == pw_p or i[14] == pw_o or i[14] == pw_z1 or i[14] == pw_o1):
                            headers = {
                            ...
                            embed = { 'title': f'적중', 'description': f'```배팅 게임 : 파워볼\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[14]}\n배팅 금액 : {i[15]}원\n─────────────\n적중 금액 : {round(i[15] * 0.98)}\n남은 금액 : {i[1] + round(i[15] * 1.98)}```', 'color' : 0x00FF00}
                            req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                            headers=headers, json={
                            'content' : f"<@{i[0]}> ", 'embed' : embed})
                            c.execute("UPDATE users SET money = money + ? where id=?", (round(i[15] * 1.98), i[0],))
        ...
        ''')
                            ...
                            data = res.json()
                            dm_channel_id = data['id']
                            embed = { 'title': f'미적중', 'description': f'```배팅 게임 : 파워볼\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[14]}\n배팅 금액 : {i[15]}원\n─────────────\n적중 금액 : 0\n남은 금액 : {i[1]}```', 'color' : 0xFF0000}
                            req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                            headers=headers, json={
                            'content' : f"<@{i[0]}> ", 'embed' : embed})
                            f = open(f"./bet_log/{i[0]}.txt", "a", encoding="utf-8-sig")
        ...
        ''')
                    ...
                    c.execute("UPDATE users SET pwball_bet_pick = ? where id=?", (None, i[0],))
                    c.execute("UPDATE users SET pwball_bet_money = ? where id=?", (None, i[0],))
                    conn.commit()
                    conn.close()
