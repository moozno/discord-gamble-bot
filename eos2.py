...
while True:
    try:
        # res = requests.get("https://bepick.net/live/result/eosball2m?_="+str(time.time()).split(".")[0], headers={
        ...
        old_data = {}
        resp=res
        with open("./eos2.json", "r", encoding="utf8") as fp:
            old_data = json.loads(fp.read().strip())
        if( old_data == res ):
            pass
        else:
            with open("./eos2.json", "w", encoding="utf8") as fp:
                eos5_o=int(res["bsum"])
                eos5_p=int(res["pb"])
                ...
                embed = { 'title': '이오스 2분   파워볼 결과', 'description': f'{res["round"]}회차', 'fields': fields }
                requests.post(이오스2분, json={'username': f'{res["round"]}회차 이오스2분 결과', "embeds": [embed]})
        # print(res)
                ...
                for i in list_a:
                    ...
                    if i[0]!=687467109243945121:
                        ...
                        print(i[34] == eos_z, i[34] == eos_o, i[34] == eos_z1, i[34] == eos_o1)


                        if(i[34] == eos_z or i[34] == eos_o or i[34] == eos_z1 or i[34] == eos_o1 or i[34] == sjd):
                            headers = {
                            ...
                            embed = { 'title': f'적중', 'description': f'```배팅 게임 : 이오스2분\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[34]}\n배팅 금액 : {i[35]}원\n─────────────\n적중 금액 : {round(i[35] * 0.98)}\n남은 금액 : {i[1] + round(i[35] * 1.98)}```', 'color' : 0x00FF00}
                            req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                            headers=headers, json={
                            'content' : f"<@{i[0]}> ", 'embed' : embed})
                            c.execute("UPDATE users SET money = money + ? where id=?", (round(i[35] * 1.98), i[0],))
        ...
        ''')
                            ...
                            data = res.json()
                            dm_channel_id = data['id']
                            embed = { 'title': f'미적중', 'description': f'```배팅 게임 : 이오스2분\n배팅 회차 : {resp["round"]}\n배팅 내역 : {i[34]}\n배팅 금액 : {i[35]}원\n─────────────\n적중 금액 : 0\n남은 금액 : {i[1]}```', 'color' : 0xFF0000}
                            req = requests.post(f"https://discordapp.com/api/v9/channels/{dm_channel_id}/messages",
                            headers=headers, json={
                            'content' : f"<@{i[0]}> ", 'embed' : embed})
                            f = open(f"./bet_log/{i[0]}.txt", "a", encoding="utf-8-sig")
        ...
        ''')
                            f.close()
                    c.execute("UPDATE users SET eos2_bet_pick = ? where id=?", (None, i[0],))
                    c.execute("UPDATE users SET eos2_bet_money = ? where id=?", (None, i[0],))
                    conn.commit()
                    conn.close()
